<script lang="ts">
  ...
  // into view. The flag is consumed on read, so a later reopen never
  // re-teleports for a click the user already honoured. An attention ring
  // lived here once (accent box-shadow, one 1.6s fade); the owner ruled it
  // noticed-and-gone noise — the expanded card and the scroll position
  // already say "here is what you clicked", twice over.
  $effect(() => {
          ...
          {#each groups as g (g.artist)}
            <div class="mi-group" role="group" aria-label={g.artist}>
              ...
              {#each g.albums as a (a.albumId)}
                ...
                <div class="mi-album" data-album-id={a.albumId}>
